import fs from 'fs';
const file = 'd:/club-youniverse-pwa/Club-Youniverse-pwa/components/DjBooth.tsx';
let dj = fs.readFileSync(file, 'utf8');
...
const handleVoteStr = `
  const handleVote = async (songId: string, currentVotes: number) => {
    ...
    await supabase.from("songs").update({ upvotes: (currentVotes || 0) + 1 }).eq("id", songId);
    setTimeout(() => { setVoteCooldowns(prev => ({ ...prev, [songId]: false })); }, 5000);
    fetchLibrary();
  };
`;

if (!dj.includes('handleVote = async')) {
    dj = dj.replace('const handleStarVote', handleVoteStr + '\n  const handleStarVote');
}

const topParts = dj.split('return (');
// The split might happen multiple times if there are inner components, but we want the last one or the main one.
// DjBooth's main return is the last one in the file because it's a big component. Wait, let's just use `indexOf('return (')`.
const returnIndex = dj.indexOf('return (');
const top = dj.substring(0, returnIndex);
...
const bottom = `return (
    <div className="fixed inset-0 bg-black text-white/90 font-mono overflow-hidden flex flex-col z-[100] selection:bg-purple-500/30">
        ...
        {/* 3. CORE INTERFACE (3-Column Dense Grid) */}
        <div className="relative z-10 flex-grow flex flex-col md:flex-row min-h-0 overflow-y-auto md:overflow-hidden gap-2 bg-black/60 p-2">
            ...
            <div className="w-full md:w-72 bg-zinc-950/80 border border-white/5 rounded-xl flex flex-col p-3 gap-3 shrink-0 overflow-y-auto">
                <div className="shrink-0">
                    ...
                    <div className="grid grid-cols-1 gap-px bg-white/5 border border-white/5 rounded-lg overflow-hidden">
                        {[
                            ...
                            <button
                                key={btn.id}
                                onClick={() => forceNextState(btn.id as string)}
                                disabled={!isCurrentDJ}
                                className={\`flex items-center justify-between px-3 py-2 transition-all group \${btn.id === radioState ? 'bg-purple-950/20' : 'bg-zinc-950 hover:bg-zinc-900'} \${!isCurrentDJ ? 'opacity-30 cursor-not-allowed filter grayscale' : ''}\`}
                            >
                ...
                <div className="flex-grow min-h-[150px] border border-yellow-500/30 bg-yellow-500/5 rounded-lg p-2 flex flex-col shrink-0">
                ...
                {/* Suno Deployment */}
                <div className="bg-black/40 border border-white/5 rounded-lg p-2.5 shrink-0 flex flex-col mt-auto">
                    ...
                    <label className={\`block py-2 text-center border-2 border-dashed rounded transition-all \${isSunoConfirmed ? 'border-purple-500/40 cursor-pointer hover:bg-purple-500/10 shadow-[inset_0_0_20px_rgba(168,85,247,0.05)]' : 'border-white/5 opacity-50 select-none'}\`}>
                        <input type="file" accept="audio/*" onChange={handleUpload as any} className="hidden" disabled={!isSunoConfirmed || isUploading} />
            ...
            <div className="flex-grow min-w-[300px] bg-zinc-950/80 border border-white/5 rounded-xl flex flex-col shrink-0 md:shrink overflow-hidden">
                <div className="p-3 border-b border-white/5 flex flex-col sm:flex-row items-center justify-between bg-black/40 gap-2 shrink-0">
                    <span className="text-[10px] font-black text-purple-400 uppercase tracking-[0.4em]">The Pool</span>
                            ...
                            <div key={song.id} className="py-2.5 flex flex-col xl:flex-row items-start xl:items-center justify-between group hover:bg-white/[0.02] -mx-4 px-4 transition-all gap-2">
                                ...
                                <div className="flex flex-wrap gap-1 flex-shrink-0 w-full xl:w-auto pl-11 xl:pl-0">
                                    ...
                                    <button
                                        ...
                                        className={\`text-[7px] font-black border border-purple-500/30 px-2 py-1 rounded text-purple-400 hover:text-white hover:bg-purple-500/20 transition-all uppercase text-center \${voteCooldowns[song.id] ? 'opacity-30 cursor-not-allowed' : ''}\`}
                                    >
                                        VOTE
                                    </button>
            ...
            {/* === COL 3: THE BOX, LIVE, PAYLOAD & FX (STACKED) (w-80 or flexed) === */}
            <div className="w-full md:w-80 flex flex-col gap-2 shrink-0">
                ...
                {/* The Box */}
                <div className="bg-zinc-950/80 border border-white/5 rounded-xl p-3 flex flex-col min-h-[160px] flex-shrink-0">
                    ...
                    <div className="flex-grow overflow-y-auto flex flex-col gap-2 pr-1 h-32">
                        ...
                        {songs.filter(s => s.status === 'in_box').length === 0 && (
                            <div className="h-full py-2 flex flex-center justify-center border border-dashed border-white/5 rounded-lg opacity-30 mt-2">
                ...
                <div className="bg-black/40 border border-white/5 rounded-lg p-2.5 flex flex-col shrink-0">
                    ...
                    <textarea
                        ...
                        className="w-full h-12 bg-black/60 border border-white/5 rounded p-2 text-[9px] text-white/80 placeholder-zinc-800 focus:outline-none focus:border-purple-500/20 transition-all font-mono resize-none mb-2"
                    />
                    <button
                        onClick={handleTtsSend}
                        disabled={isSending || !ttsInput.trim()}
                        className="w-full py-1.5 bg-white text-black text-[8px] font-black uppercase tracking-widest hover:invert transition-all rounded"
                    >
                        {isSending ? 'Sending...' : 'Transmit Payload'}
                    </button>
                </div>

                {/* Pulse Monitor (Screen FX) */}
                <div className="bg-black/40 border border-white/5 rounded-lg p-2.5 shrink-0 flex-grow min-h-0 min-h-[100px] flex flex-col">
                    <span className="text-[7px] font-black text-zinc-700 uppercase tracking-[0.4em] mb-2 block">Screen FX (Pulse Monitor)</span>
                    <div className="relative border border-white/5 rounded bg-zinc-950/20 overflow-hidden h-full flex items-center justify-center p-1">
                        <div className="absolute inset-0 grid grid-cols-5 grid-rows-2 opacity-[0.03] pointer-events-none">
                            {Array.from({ length: 10 }).map((_, i) => <div key={i} className="border border-white" />)}
                        </div>
                        <div className="z-10 grid grid-cols-5 gap-1 w-full h-full">
                            {["Confetti", "Glitch", "Shake", "Pulse", "Static", "Invert", "Hue", "Blur", "Pixel", "Neon"].map(fx => (
                                <button
                                    key={fx}
                                    onClick={() => sendSiteCommand("trigger_fx", { fx })}
                                    disabled={!canControl}
                                    className={\`px-1 py-1.5 sm:py-2 bg-black/50 border rounded text-[6px] md:text-[8px] font-black uppercase transition-all shadow-sm flex items-center justify-center \${canControl
                                        ? 'border-white/10 text-zinc-400 hover:border-purple-500/50 hover:bg-white/10 hover:text-white hover:scale-105 active:scale-95'
