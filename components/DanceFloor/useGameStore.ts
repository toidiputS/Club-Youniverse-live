...
interface GameState {
  ...
  songLeaderboard: { title: string; artist: string; stars: number }[];
  leaderboardTab: 'youniversal' | 'songs';
  setLeaderboardTab: (tab: 'youniversal' | 'songs') => void;
  
  // Actions
  connect: () => void;
  disconnect: () => void;
  claimSlot: (slotId: string, name?: string, userId?: string) => void;
...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  fetchLeaderboard: async () => {
    try {
      ...
      set({ 
        ...
        songLeaderboard: (songData || []).map(s => ({
            title: s.title,
    ...
    ws.onmessage = (event) => {
      ...
      if (data.type === 'init') {
        ...
        set((state) => {
          ...
          const newForces: Record<string, ForceField> = {};
          if (data.forceFields) {
            data.forceFields.forEach((f: ForceField) => {
              newForces[f.id] = f;
            });
  ...
  claimSlot: (slotId: string, name?: string, userId?: string) => {
    const { ws } = get();
    if (ws && ws.readyState === WebSocket.OPEN) {
      set({ joinError: null });
      ws.send(JSON.stringify({ type: 'claim_slot', slotId, name, userId }));
    }
  },
